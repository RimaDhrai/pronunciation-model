...
public class OllamaService implements IOllamaService {
    ...
    public String generatePhrase(String lang, String level) {
        ...
        if ("fr".equals(lang)) {
            ...
            prompt = "GÃ©nÃ¨re une phrase franÃ§aise de " + wc + " sur un sujet quotidien (voyage, nourriture, famille, travail, mÃ©tÃ©o, sport).";
        } else {
            ...
            prompt = "Generate an English sentence of " + wc + " about a daily topic (travel, food, family, work, weather, sport).";
        }
    ...
    public String generateFeedback(
        ...
        List<Map<String, Object>> ops =
                (List<Map<String, Object>>) scoreResult.get("ops");
        ...
        if (errors.length() == 0) errors.append("  - Aucune erreur majeure\n");

        // Si hallucination suspectÃ©e, ajouter un contexte explicite pour que le coach soit honnÃªte
    ...
    public String buildChatbotUserContent(
        ...
        String content = (userText == null || userText.isBlank())
                ? "(silence â€” encourage user to speak)" : userText;
    ...
    /** Same as generateChatbotResponse but streams tokens via onToken callback. Returns full response. */
    public String streamChatbotResponse(
        ...
        try {
            ...
            if (full.isEmpty()) return "âš ï¸ RÃ©ponse indisponible";
        }
    ...
    private String buildRoleplaySystemPrompt(String lang, String level, String scenario) {
        ...
        String roleContext = switch (scenario) {
            case "customs" -> fr
                ? ("Douanier CDG ðŸ›‚. %s. Max 2 phrases. Passeport, sÃ©jour, bagages. Si [Prononciation incertaine: X] : corrige en restant dans le rÃ´le. Avance le scÃ©nario.").formatted(levelHint)
                : ("UK Border Control Heathrow ðŸ›‚. %s. Max 2 sentences. Passport, stay, luggage. If [Uncertain pronunciation: X] : correct subtly in character. Advance scenario.").formatted(levelHint);
            case "interview" -> fr
                ? ("Manager RH entretien ðŸ’¼. %s. Max 2 phrases. Parcours, motivation, compÃ©tences. Si [Prononciation incertaine: X] : corrige discrÃ¨tement. Avance l'entretien.").formatted(levelHint)
                : ("Hiring manager interview ðŸ’¼. %s. Max 2 sentences. Background, motivation, skills. If [Uncertain pronunciation: X] : correct subtly. Advance interview.").formatted(levelHint);
            case "restaurant" -> fr
                ? ("Serveur restaurant parisien ðŸ½. %s. Max 2 phrases. Accueil â†’ commande â†’ addition. Si [Prononciation incertaine: X] : corrige dans le rÃ´le.").formatted(levelHint)
                : ("London restaurant waiter ðŸ½. %s. Max 2 sentences. Welcome â†’ order â†’ bill. If [Uncertain pronunciation: X] : correct subtly in character.").formatted(levelHint);
    ...
    public String generateLevelTestPhrase(String lang, String soundContext, String level) {
        ...
        String system = "fr".equals(lang)
            ? "Tu gÃ©nÃ¨res UNE phrase franÃ§aise parlÃ©e (" + levelHint + "). INTERDIT d'utiliser des formules d'introduction (pas de 'Voici', pas de 'Bien sÃ»r'). RÃ©ponds DIRECTEMENT avec la phrase."
            : "Generate ONE spoken English sentence (" + levelHint + "). FORBIDDEN to use introduction phrases (no 'Here is', no 'Sure'). Reply DIRECTLY with the sentence.";
        String prompt = "fr".equals(lang)
    ...
    public String generateLevelTestSynthesis(String lang,
        ...
        if ("fr".equals(lang)) {
            if (avg >= 75) return "TrÃ¨s bon niveau (" + finalLevel + ") ! Tu es sur la bonne voie.";
            if (avg >= 55) return "Bon niveau gÃ©nÃ©ral (" + finalLevel + "). Quelques sons mÃ©ritent plus de pratique.";
            return "Des bases solides Ã  renforcer. Pratique rÃ©guliÃ¨rement les sons ciblÃ©s pour progresser.";
        } else {
            if (avg >= 75) return "Very good pronunciation (" + finalLevel + ")! Keep up the great work.";
            if (avg >= 55) return "Good overall level (" + finalLevel + "). A few sounds need more practice.";
    ...
    private String cleanLevelTestPhrase(String raw) {
        ...
        for (String line : raw.strip().split("\n")) {
            ...
            if (lower.matches(".*\\b(reactdom\\.render|function\\s+\\w+|class\\s+\\w+|import\\s+|export\\s+|<[^>]+>|\\{.*\\}|;).*") ) {
                continue;
            }
    ...
    private String buildLevelTestFeedbackFallback(String lang, String soundLabel,
        ...
        if ("fr".equals(lang)) {
            if (score >= 75) return "Excellent ! Tu prononces trÃ¨s bien le son " + soundLabel + ". Continue ! ðŸŒŸ";
            if (score >= 55) return "Bien jouÃ© ! Le son " + soundLabel + " est presque parfait. RÃ©pÃ¨te : " + w0 + " ðŸ‘";
            return "Le son " + soundLabel + " est difficile. EntraÃ®ne-toi avec : " + w0 + ". Ã‡a viendra ! ðŸ’ª";
        } else {
            if (score >= 75) return "Excellent! You nailed the " + soundLabel + " sound. Keep it up! ðŸŒŸ";
            if (score >= 55) return "Well done! The " + soundLabel + " is almost perfect. Practice: " + w0 + " ðŸ‘";
            return "The " + soundLabel + " is challenging. Practice: " + w0 + ". You'll get there! ðŸ’ª";
        }
    }

    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // PUBLIC RAW CALL â€” used by ExerciseAiController for revision/phonetic phrases
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // APPEL OLLAMA /api/chat  (single implementation â€” delegates from 2-message helper)
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    /** All Ollama HTTP calls go through here. Used by chatbot (with full history) and all other callers. */
    ...
    public String generateExerciseFeedback(String lang, String level, String type,
        ...
        String prompt = fr
                ? String.format("Niveau %s â€” exercices de %s. Score : %d/100 (%d/%d bonnes rÃ©ponses). Erreurs : %s. Donne un feedback motivant.",
                    level, type, score, correct, total, errorsDetail.isBlank() ? "aucune" : errorsDetail)
                : String.format("Level %s â€” %s exercises. Score: %d/100 (%d/%d correct). Errors: %s. Give motivating feedback.",
